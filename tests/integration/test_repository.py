...
def test_repository_can_save_a_batch(session):
    ...
    rows = list(
        session.execute("SELECT ref, sku, _purchased_quantity, eta FROM 'batches'")
    )
...
def test_repository_can_retrieve_a_batch_with_allocations(session):
    ...
    repo = repository.SqlAlchemyRepository(session)
    retrieved = repo.get('batch_1')

    expected = model.Batch(ref='batch_1', sku='GENERIC-SOFA', qty=100, eta=None)
